...
class list {
 ...
 public:
  ...
  list(size_t n, const T& val, const allocator_type& alloc = allocator_type());
  // range
...
template <class T, class Alloc>
void list<T, Alloc>::del_node(node* ptr) {
  mrsuyi::destroy_at(ptr);
  alloc_.deallocate(ptr, 1);
}
// node/size manipulations
template <class T, class Alloc>
typename list<T, Alloc>::node* list<T, Alloc>::insert(node* pos, node* one) {
  return insert(pos, one, one, 1);
}
template <class T, class Alloc>
typename list<T, Alloc>::node* list<T, Alloc>::insert(node* pos,
                                                      node* head,
                                                      node* tail,
                                                      size_t cnt) {
  size_ += cnt;
...
template <class T, class Alloc>
typename list<T, Alloc>::node* list<T, Alloc>::yield(node* one) {
  return yield(one, one, 1);
}
template <class T, class Alloc>
typename list<T, Alloc>::node* list<T, Alloc>::yield(node* head,
                                                     node* tail,
                                                     size_t cnt) {
  size_ -= cnt;
...
template <class T, class Alloc>
typename list<T, Alloc>::node* list<T, Alloc>::erase(node* one) {
  auto res = one->nxt;
  del_node(yield(one));
  return res;
}
...
template <class T, class Alloc>
void list<T, Alloc>::assign(size_t n, const T& val) {
...
template <class T, class Alloc>
size_t list<T, Alloc>::max_size() const noexcept {
...
template <class T, class Alloc>
void list<T, Alloc>::resize(size_t size) {
  resize(size, T());
}
template <class T, class Alloc>
void list<T, Alloc>::resize(size_t size, const T& val) {
  if (size_ < size)
...
typename list<T, Alloc>::iterator list<T, Alloc>::emplace(const_iterator pos,
                                                          Args&&... args) {
  return iterator(insert(pos.node_, new_node(mrsuyi::forward<Args>(args)...)));
}
template <class T, class Alloc>
template <class... Args>
...
template <class T, class Alloc>
void list<T, Alloc>::insert(const_iterator pos, size_t n, const T& val) {
  for (size_t i = 0; i < n; ++i)
    insert(pos.node_, new_node(val));
}
template <class T, class Alloc>
template <class InputIterator>
...
template <class T, class Alloc>
template <class Compare>
void list<T, Alloc>::merge(list&& other, Compare cmp) {
  size_ += other.size_;
...
template <class T, class Alloc>
void list<T, Alloc>::splice(const_iterator pos,
                            list& other,
                            const_iterator it) {
...
template <class T, class Alloc>
bool operator==(const list<T, Alloc>& lhs, const list<T>& rhs) {
