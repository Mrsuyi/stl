...
constexpr shared_ptr<T>::shared_ptr() noexcept
    : stored_(nullptr), ctrl_(new ctrl<T>(nullptr, 1)) {}
template <class T>
...
// copy
template <class T>
shared_ptr<T>::shared_ptr(const shared_ptr& x) noexcept
    : stored_(x.stored_), ctrl_(x.ctrl_) {
  ++(ctrl_->shared);
}
template <class T>
template <class U>
shared_ptr<T>::shared_ptr(const shared_ptr<U>& x) noexcept
    : stored_(x.ptr_), ctrl_(x.ctrl_) {
  ++(ctrl_->shared);
}
// copy from weak
template <class T>
...
shared_ptr<T>::shared_ptr(const weak_ptr<U>& x)
    : stored_(x.ctrl_->managed), ctrl_(x.ctrl_) {
  ++(ctrl_->shared);
}
// move
template <class T>
shared_ptr<T>::shared_ptr(shared_ptr&& x) noexcept
    : stored_(x.stored_), ctrl_(x.ctrl_) {
  x.ctrl_ = new ctrl<T>(nullptr, 1);
}
template <class T>
template <class U>
shared_ptr<T>::shared_ptr(shared_ptr<U>&& x) noexcept
    : stored_(x.stored_), ctrl_(x.ctrl_) {
  x.ctrl_ = new ctrl<T>(nullptr, 1);
}
// move from unique
...
shared_ptr<T>::shared_ptr(unique_ptr<U, Del>&& x)
    : stored_(x.get()),
      ctrl_(new ctrl<T>(x.get(), 1, 0, move(x.get_deleter()))) {
  x.release();
}
// alias
template <class T>
...
shared_ptr<T>::~shared_ptr() {
  --(ctrl_->shared);
  if (ctrl_->shared == 0 && ctrl_->weak == 0)
    delete ctrl_;
}
...
// copy
template <class T>
shared_ptr<T>& shared_ptr<T>::operator=(const shared_ptr& x) noexcept {
...
template <class T>
template <class U>
shared_ptr<T>& shared_ptr<T>::operator=(const shared_ptr<U>& x) noexcept {
  shared_ptr<T>(x) > swap(*this);
  return *this;
}
template <class T>
template <class U, class Del>
shared_ptr<T>& shared_ptr<T>::operator=(unique_ptr<U, Del>&& x) {
  shared_ptr<T>(x).swap(*this);
  return *this;
}
...
template <class T>
template <class U, class Del>
void shared_ptr<T>::reset(U* p, Del del) {
  shared_ptr<T>(p, del).swap(*this);
}

template <class T>
template <class U, class Del, class Alloc>
void shared_ptr<T>::reset(U* p, Del del, Alloc alloc) {
  shared_ptr<T>(p, del, alloc).swap(*this);
}

template <class T>
template <class U>
bool shared_ptr<T>::owner_before(const shared_ptr<U>& x) const {
