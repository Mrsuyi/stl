...
// copy
template <class T>
weak_ptr<T>& weak_ptr<T>::operator=(const weak_ptr& x) noexcept {
  weak_ptr(x).swap(*this);
}
template <class T>
template <class U>
weak_ptr<T>& weak_ptr<T>::operator=(const weak_ptr<U>& x) noexcept {
  weak_ptr(x).swap(*this);
}
// from shared_ptr
template <class T>
template <class U>
weak_ptr<T>& weak_ptr<T>::operator=(const shared_ptr<U>& x) noexcept {
  weak_ptr(x).swap(*this);
}
// move
template <class T>
weak_ptr<T>& weak_ptr<T>::operator=(weak_ptr&& x) noexcept {
  weak_ptr(x).swap(*this);
}
template <class T>
template <class U>
weak_ptr<T>& weak_ptr<T>::operator=(weak_ptr<U>&& x) noexcept {
  weak_ptr(x).swap(*this);
}

// get
template <class T>
long int weak_ptr<T>::use_count() const noexcept {
  return ctrl_->shared;
}
...
template <class T>
shared_ptr<typename weak_ptr<T>::element_type> weak_ptr<T>::lock() const
    noexcept {
...
template <class T>
template <class U>
bool weak_ptr<T>::owner_before(const weak_ptr<U>& x) const {
  return ctrl_->managed < x.ctrl_->managed;
}

template <class T>
template <class U>
bool weak_ptr<T>::owner_before(const shared_ptr<U>& x) const {
  return ctrl_->managed < x.ctrl_->managed;
}
