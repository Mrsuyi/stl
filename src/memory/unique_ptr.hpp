...
template <class T, class D>
constexpr unique_ptr<T, D>::unique_ptr(std::nullptr_t) noexcept
    : unique_ptr() {}
...
template <class T, class D>
unique_ptr<T, D>::unique_ptr(unique_ptr&& x) noexcept
    : ptr_(x.release()), del_(move(x.del_)) {}
...
template <class T, class D>
unique_ptr<T, D>& unique_ptr<T, D>::operator=(unique_ptr&& x) noexcept {
  reset(x.release());
  del_ = move(x.del_);
  return *this;
}
...
template <class T, class D>
const typename unique_ptr<T, D>::deleter_type& unique_ptr<T, D>::get_deleter()
    const noexcept {
  return del_;
}
...
template <class T, class D>
typename std::add_lvalue_reference<typename unique_ptr<T, D>::element_type>::
    type unique_ptr<T, D>::operator*() const {
  return *get();
}
...
template <class T, class D>
void unique_ptr<T, D>::swap(unique_ptr& x) noexcept {
