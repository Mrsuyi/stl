...
advance(InputIterator& it, Distance n)
{
    ...
    __advance(it, diff, __iterator_category(it));
}
template <class InputIterator, class Distance>
void
...
{
    __distance(first, last, __iterator_category(first));
}
template <class InputIterator>
typename iterator_traits<InputIterator>::difference_type
...
template <class ForwardIterator>
ForwardIterator
prev(ForwardIterator it, typename iterator_traits<ForwardIterator>::difference_type n = 1)
{
    advance(it, -n);
    return it;
}
