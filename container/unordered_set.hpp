...
public:
    ...
    size_t size() const noexcept { return ht_.size(); }
    size_t max_size() const noexcept { return ht_.max_size(); }
    // modifiers
    ...
    void insert(std::initializer_list<Key> il) { insert(il.begin(), il.end()); }
    template <class... Args>
...
public:
    ...
    explicit unordered_multiset(size_t bucket_suggest, const Hash& hs = Hash(),
                                const KeyEqual& ke = KeyEqual(),
                                const Allocator& al = Allocator())
        : ht_(bucket_suggest, hs, ke, al)
    {
    ...
    // range
    template <class InputIt>
    unordered_multiset(InputIt first, InputIt last) : ht_()
    {
        for (; first != last; ++first) insert(*first);
    }
    // il
    unordered_multiset(std::initializer_list<Key> il)
        : unordered_multiset(il.begin(), il.end())
    {
    }
    ...
    mrsuyi::pair<iterator, bool> emplace(Args... args)
    {
        return ht_.emplace_equal(mrsuyi::forward<Args>(args)...);
    }
    ...
    size_t erase(const Key& key) { return ht_.erase_equal(key); }
    void swap(unordered_multiset& other) { ht_.swap(other.ht_); }
    // lookup
    size_t count(const Key& key) const { return ht_.count_equal(key); }
    iterator find(const Key& key) { return ht_.find(key); }
    const_iterator find(const Key& key) const { return ht_.find(key); }
    mrsuyi::pair<iterator, iterator> equal_range(const Key& key)
    {
           ...
           const unordered_multiset<Key, Hash, KeyEqual, Allocator>& rhs)
{
    ...
    {
        auto cnt = lhs.count(*it);
        if (cnt != rhs.count(*it)) return false;
        mrsuyi::advance(it, cnt);
    }
    return true;
}
